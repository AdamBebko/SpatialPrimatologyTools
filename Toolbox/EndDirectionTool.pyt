...
class Tool(object):
    ...
    def getParameterInfo(self):
        ...
        params = [inFileParam, indivName, dateField, lostDay, outFileParam]
        return params
    ...
    def execute(self, parameters, messages):
        ...
        indivName = parameters[1].valueAsText
        messages.addMessage("indivName = " + indivName)
        
        dateField = parameters[2].valueAsText
        messages.addMessage("dateField = " + dateField)
        
        lostDay = parameters[3].valueAsText
        messages.addMessage("lostDay = " + lostDay)
        
        outFile = parameters[4].valueAsText
        messages.addMessage("outFile = " + outFile)
        
        date = None
        name = None
        coordsList = []
        with arcpy.da.SearchCursor(inFile, ["SHAPE@XY", indivName, dateField]) as cursor: #@UndefinedVariable
            for row in cursor:
                if not date:
                    date = row[2]
                if not name:
        ...
        try:
            ...
            if len(coordsList) >= 2:   
                outCursor.insertRow([arcpy.Polyline(a1), name, date, lostDay, 1])
            if len(coordsList) >= 3:   
                outCursor.insertRow([arcpy.Polyline(a2), name, date, lostDay, 2])
            if len(coordsList) >= 4:   
                outCursor.insertRow([arcpy.Polyline(a3), name, date, lostDay, 3])
            if i != 0:   
                outCursor.insertRow([arcpy.Polyline(aAvg), name, date, lostDay, "Avg"])
